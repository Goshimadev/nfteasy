...
import { Babel__factory, Babel } from "../typechain-types"
...
describe("babel", () => {
  beforeEach(async () => {
    ...
    babelFactory = (await ethers.getContractFactory("Babel", deployer)) as Babel__factory
  ...
  describe("minting", async () => {
    ...
    const tokenURI = "https://velopace/images/shares/giving.png"
    ...
    it("should mint a new book when anyone calls it", async function () {
      ...
      await expect(babel.connect(other).setTokenURI(tokenId, tokenURI)).to.be.revertedWith(
    ...
    it("other accounts cannot mint tokens", async () => {
      ...
      await expect(babel.connect(other).safeMint(other.address, tokenURI)).to.be.revertedWith(
        "Ownable: caller is not the owner"
      )
    })
  })
  ...
  describe("updating", async () => {
    it("update tokenURI works as expected", async () => {
      ...
      await expect(babel.connect(deployer).updateTokenURI(tokenId, newTokenURI)).to.be.revertedWith(
  ...
  describe("burning", async () => {
    it("holders can burn their tokens", async () => {
      ...
      await expect(babel.ownerOf(tokenId)).to.be.revertedWith("ERC721: owner query for nonexistent token")
    ...
    it("cannot burn if not token owner", async () => {
      ...
      await expect(babel.connect(deployer).burn(tokenId)).to.be.revertedWith("function call to a non-contract account")
