...
import { BabelAuthMint__factory, BabelAuthMint } from "../typechain-types"
...
describe("babelAuthMint", () => {
  beforeEach(async () => {
    ...
    babelAuthMintFactory = (await ethers.getContractFactory("BabelAuthMint", Alice)) as BabelAuthMint__factory
  ...
  describe("assigning minter role", async () => {
    it("Giving Bob minter role", async () => {
      ...
      await expect(await babelAuthMint.hasRole(await babelAuthMint.MINTER_ROLE(), Bob.address)).to.equal(true)
    })
  })
  ...
  describe("minting", async () => {
    ...
    it("Bob (minter) is able to mint tokens", async () => {
      ...
      await babelAuthMint.connect(Bob).claim(Bob.address, tokenId, sig)
      await expect(await babelAuthMint.ownerOf(tokenId)).to.equal(Bob.address)
    })
