...
import { BabelMerkleMint__factory, BabelMerkleMint } from "../typechain-types"
...
describe("babelMerkleMint", () => {
  beforeEach(async () => {
    ...
    babelMerkleMintFactory = (await ethers.getContractFactory("BabelMerkleMint", Alice)) as BabelMerkleMint__factory
    members = {
      "1": Charlie.address,
      "2": Derek.address,
    }
    merkleTree = new MerkleTree(
      Object.entries(members).map((token) => hashToken(...token)),
      keccak256,
      { sortPairs: true }
    )

    babelMerkleMint = (await babelMerkleMintFactory.connect(Alice).deploy(merkleTree.getHexRoot())) as BabelMerkleMint
  ...
  it("Charlie and Derek claim their token", async () => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      const proof = merkleTree.getHexProof(hashToken(tokens[i][0], tokens[i][1]))
      await babelMerkleMint.claim(tokens[i][1], tokens[i][0], proof)
      await expect(await babelMerkleMint.ownerOf(tokens[i][0])).to.equal(tokens[i][1])
    }
  })
})
