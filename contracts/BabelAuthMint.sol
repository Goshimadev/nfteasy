...
contract BabelAuthMint is ERC721, EIP712, AccessControl {
    ...
    constructor() ERC721("Babel", "LIB") EIP712("Babel", "1.0.0") {
    ...
    function claim(
        ...
        require(_verify(_hash(tokenId, to), signature), "Invalid signature");
        _safeMint(to, tokenId);
    }

    function _hash(uint256 id, address addr) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(keccak256("NFT(uint256 tokenId,address account)"), id, addr)));
    }
